...
public class Main_16927 {
	...
	static int origianlArr[][];		 //원본배열
	static int rotationArr[][];  	 //돌릴배열
	static ArrayList<Integer> rotationCount;
		...
		try {
			...
			for(int i=0; i<N; i++) {				  //배열에 입력받은 값 넣기
				...
				for(int j=0; j<M; j++) {
					...
					origianlArr[i][j] = Integer.parseInt(st.nextToken());
	...
	private static void rotationArr(int n, int m, int num) {
		...
		if(n < origianlArr.length/2 || m < origianlArr[0].length/2) {	// 재귀 메서드를 끝내는 조건문
			copyArray(rotationArr,origianlArr);						// 제한사항중 n m 둘다 홀수일 수는 없으므로 ||조건
			return;
		}
		...
		for(int i=0; i<numBorder; i++) {                // 테두리 수 만큼 반복
			...
			if(i<fixedN) {								// 4개의 변중 오른쪽 아래에서 오른쪽 위 방향으로 이동
				rotationArr[n-1][m] = origianlArr[n][m];
				n--;
			}else if(i<fixedN+fixedM) {					// 4개의 변중 오른쪽 위에서 왼쪽 위 방향으로 이동
				rotationArr[n][m-1] = origianlArr[n][m];
				m--;	
